...
// ------------------------------ FIELDS ------------------------------
    ...
    protected final Map<String, ExternalTable> tables = new LinkedHashMap<>();
     ...
     */
    private Set<String> altered = new LinkedHashSet<>();
        ...
        if (t != null) {
            ...
            if (obj.getType() == DbObject.TABLE_OR_VIEW) {
                ...
                if (table != null) {
                    ...
                    indexes.entrySet().removeIf(entry -> entry.getKey().startsWith(prefixed));
